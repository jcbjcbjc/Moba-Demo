    ...
    execute: function () {
      ...
      _export("ItemManager", ItemManager = class ItemManager {
        ...
        AddItem(itemId, count) {
          let itemList = (_crd && User === void 0 ? (_reportPossibleCrUseOfUser({
            error: Error()
          }), User) : User).Instance.user.bag; // 先查询当前商品堆叠数量

          let heap = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
            error: Error()
          ...
          if (count > 0) {
            for (let i = 0; i < (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
              error: Error()
            }), DataManager) : DataManager).Instance.gameConfig.BagMaxGridCount - itemList.length && count > 0; i++) {
              if (count >= heap) {
                let build = new NItem();
                build.itemId = itemId;
                build.count = heap;
                ...
                build.itemId = itemId;
                build.count = count;
                itemList.push(build); // 所有物品已加入 背包
        ...
        RemoveItem(itemId, count) {
          let items = (_crd && User === void 0 ? (_reportPossibleCrUseOfUser({
            error: Error()
