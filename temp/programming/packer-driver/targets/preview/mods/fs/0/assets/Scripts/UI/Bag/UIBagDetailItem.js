      ...
      _export("UIBagDetailItem", UIBagDetailItem = (_dec = ccclass('UIBagDetailItem'), _dec2 = property(Label), _dec3 = property(Label), _dec4 = property(Label), _dec5 = property(Label), _dec6 = property(Sprite), _dec7 = property(Node), _dec8 = property(Node), _dec(_class = (_class2 = (_temp = class UIBagDetailItem extends Component {
        ...
        SetItemInfo(item) {
          ...
          return _asyncToGenerator(function* () {
            _this.define = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
              error: Error()
            ...
            _this.imgSprite.spriteFrame = yield (_crd && LoadResUtil === void 0 ? (_reportPossibleCrUseOfLoadResUtil({
              error: Error()
            }), LoadResUtil) : LoadResUtil).load_local_sprite(_this.define.Icon + '/spriteFrame');
            _this.nameLabel.string = _this.define.Name;
            _this.countLabel.string = '数量：' + item.count;
            _this.priceLabel.node.active = item.isTran;

            var descPos = _this.descLabel.node.getPosition();

            if (item.isTran) {
              //交易
              _this.priceLabel.string = '价格：' + item.money;

              _this.descLabel.node.setPosition(descPos.x, -118, descPos.z);

              _this.tranButton.active = false;
              _this.cancelTranButton.active = true;
            } else {
              //未交易设置描述位置
              _this.descLabel.node.setPosition(descPos.x, -78, descPos.z);

              _this.tranButton.active = true;
              _this.cancelTranButton.active = false;
            }

            _this.descLabel.string = '描述：' + _this.define.Description;
          })();
        }
