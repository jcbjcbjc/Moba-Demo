    ...
    execute: function () {
      ...
      _export("UIBattle", UIBattle = (_dec = ccclass('UIBattle'), _dec2 = property(Node), _dec3 = property([Node]), _dec4 = property(Label), _dec5 = property([SpriteFrame]), _dec6 = property(Sprite), _dec7 = property(Label), _dec8 = property(Label), _dec9 = property(Node), _dec10 = property(Node), _dec11 = property(Sprite), _dec12 = property(RichText), _dec13 = property(Prefab), _dec14 = property(Label), _dec(_class = (_class2 = (_temp = class UIBattle extends Component {
        ...
        start() {
          ...
          for (var i = 0; i < this.controlActiveNodeArr.length; i++) {
            var node = this.controlActiveNodeArr[i];
        ...
        updateMyChatLastRichText() {
          var len = (_crd && ChatManager === void 0 ? (_reportPossibleCrUseOfChatManager({
            error: Error()
          }), ChatManager) : ChatManager).Instance.gameMessages.length;

          if (len > 0) {
            var chatMessage = (_crd && ChatManager === void 0 ? (_reportPossibleCrUseOfChatManager({
              error: Error()
        ...
        updateLiveChatLastMsg() {
          ...
          if (len > 0) {
            ...
            var liveMsgStr = (_crd && BattleGlobal === void 0 ? (_reportPossibleCrUseOfBattleGlobal({
              error: Error()
            ...
            var uiLiveMsg = node.getComponent(_crd && UILiveMsg === void 0 ? (_reportPossibleCrUseOfUILiveMsg({
              error: Error()
            }), UILiveMsg) : UILiveMsg);
            uiLiveMsg.Text = liveMsgStr;
            var heightRange = this.windowSize.height / 2 - 30; //高度正负范围随机
        ...
        OnTouchMove(e) {
          ...
          var angle = Math.round((_crd && MathUtil === void 0 ? (_reportPossibleCrUseOfMathUtil({
            error: Error()
          ...
          var cameraPos = (_crd && BattleManager === void 0 ? (_reportPossibleCrUseOfBattleManager({
            error: Error()
          }), BattleManager) : BattleManager).Instance.camera.worldPosition;
          var speed = 2.5;
          var vx = cameraPos.x + (teamType2 == (_crd && TeamType2 === void 0 ? (_reportPossibleCrUseOfTeamType({
            error: Error()
          }), TeamType2) : TeamType2).Blue ? -1 : 1) * (rockerSpeedVo.x * 0.001) * speed;
          var vz = cameraPos.z + (teamType2 == (_crd && TeamType2 === void 0 ? (_reportPossibleCrUseOfTeamType({
            error: Error()
        ...
        update() {
          ...
          if (this.isEndFlag && this.isMoveFlag) {
            var jgTime = (new Date().getTime() - this.moveTime) / 1000; //距离最后一次移动间隔时间(s)
          ...
          if (this.moveLiveMsgList.length > 0) {
            for (var i = 0; i < this.moveLiveMsgList.length; i++) {
              var node = this.moveLiveMsgList[i];
              var richTextNode = node.getComponent(_crd && UILiveMsg === void 0 ? (_reportPossibleCrUseOfUILiveMsg({
                error: Error()
              }), UILiveMsg) : UILiveMsg).richText.node;
              var richTextWidth = richTextNode.getComponent(UITransform).contentSize.width; //文字宽

              var pos = richTextNode.position;
              var x = pos.x - 1;
