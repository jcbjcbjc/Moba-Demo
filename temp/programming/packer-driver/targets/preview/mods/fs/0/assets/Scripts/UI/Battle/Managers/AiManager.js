      ...
      _export("AiManager", AiManager = class AiManager {
        ...
        */
        PathWay() {
          ...
          var endLogicVec3 = this.targetLogicVec3List[this.targetLogicVec3List.length - 1];
          ...
          var index = this.finishPathWayIndex + 1;

          if (index < this.targetLogicVec3List.length) {
            var targetLogicVec3 = this.targetLogicVec3List[index]; //获取目标方向

            var targetDirZ2 = targetLogicVec3.z < this.Owner.logicPosition.z ? 1 : -1;
            var targetDirX2 = targetLogicVec3.x < this.Owner.logicPosition.x ? 1 : -1;
            ...
            var vx = this.Owner.logicPosition.x + rockerSpeedVo.x * this.Owner.attributes.Speed;
            var vz = this.Owner.logicPosition.z + rockerSpeedVo.y * this.Owner.attributes.Speed;
        ...
        */
        ...
        AiExecute() {
          var creatureClass = this.Owner.characterDefine.Class.toString(); //移动车
          ...
          if (!this.Owner.IsDeath && creatureClass == CharacterClass[CharacterClass.Cart]) {
            ...
            if (!this.pathWayFlag && !this.isFinishPathWay) {
              ...
              var npcMoveWayArr = (_crd && BattleManager === void 0 ? (_reportPossibleCrUseOfBattleManager({
                error: Error()
              }), BattleManager) : BattleManager).Instance.npcMoveWayNode.children;

              for (var i = 0; i < npcMoveWayArr.length; i++) {
                var node = npcMoveWayArr[i];
                var name = node.name;
          ...
          if (!this.Owner.IsDeath && creatureClass == CharacterClass[CharacterClass.Soldier]) {
            if (this.Owner.isGuardCart) {
              ...
              var dir = this.Owner.teamType2 == (_crd && TeamType2 === void 0 ? (_reportPossibleCrUseOfTeamType({
                error: Error()
              }), TeamType2) : TeamType2).Blue ? 1 : -1;
              var cart = this.Owner.teamType2 == (_crd && TeamType2 === void 0 ? (_reportPossibleCrUseOfTeamType({
                error: Error()
        ...
        SearchTargetMove() {
          ...
          var targetTeamType2 = this.Owner.teamType2 == (_crd && TeamType2 === void 0 ? (_reportPossibleCrUseOfTeamType({
            error: Error()
