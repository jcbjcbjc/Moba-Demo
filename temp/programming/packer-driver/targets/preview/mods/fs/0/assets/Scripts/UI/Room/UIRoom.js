      ...
      _export("UIRoom", UIRoom = (_dec = ccclass('UIRoom'), _dec2 = property(Sprite), _dec3 = property(Label), _dec4 = property(Sprite), _dec5 = property(Label), _dec6 = property(Sprite), _dec7 = property(Label), _dec8 = property(Sprite), _dec9 = property(Label), _dec10 = property(Sprite), _dec11 = property(Label), _dec12 = property(Sprite), _dec13 = property(Label), _dec14 = property(Label), _dec15 = property(Button), _dec16 = property(Button), _dec17 = property(Button), _dec18 = property(Sprite), _dec19 = property(Node), _dec20 = property(Node), _dec21 = property(Node), _dec22 = property(Node), _dec23 = property(Node), _dec(_class = (_class2 = (_temp = _class3 = class UIRoom extends Component {
        ...
        OnMyRoom_UI(param) {
          ...
          return _asyncToGenerator(function* () {
            _this.myNicknameLabel1.node.active = false;
            _this.myNicknameLabel2.node.active = false;
            _this.myNicknameLabel3.node.active = false;
            _this.enemyNicknameLabel1.node.active = false;
            _this.enemyNicknameLabel2.node.active = false;
            _this.enemyNicknameLabel3.node.active = false;
            _this.mykickOutButton2.active = false;
            _this.mykickOutButton3.active = false;
            _this.enemykickOutButton1.active = false;
            _this.enemykickOutButton2.active = false;
            _this.enemykickOutButton3.active = false;
            var room = param[0];
            _this.room = room;
            var roomOwnerFlag = room.userId == (_crd && User === void 0 ? (_reportPossibleCrUseOfUser({
              error: Error()
            }), User) : User).Instance.user.id;
            _this.myInviteButton.node.active = roomOwnerFlag;
            _this.enemyInviteButton.node.active = roomOwnerFlag;
            _this.startGameButton.node.active = roomOwnerFlag;
            var myList = room.my;
            var enemyList = room.enemy;
            _this.roomIdLabel.string = '房间id：' + room.roomId;
            var this_ = _this;

            for (var i = 0; i < _this.teamNum; i++) {
              if (i < myList.length) {
                var my = myList[i];
                _this['mySprite' + (i + 1)].spriteFrame = yield (_crd && LoadResUtil === void 0 ? (_reportPossibleCrUseOfLoadResUtil({
                  error: Error()
                ...
                _this['myNicknameLabel' + (i + 1)].string = my.nickName;
                _this['myNicknameLabel' + (i + 1)].node.active = true;

                if (i > 0 && roomOwnerFlag) {
                  console.log('mykickOutButton' + (i + 1));
                  _this['mykickOutButton' + (i + 1)]['roomUser'] = my;
                  _this['mykickOutButton' + (i + 1)].active = true;
                } //头像点击


                _this['mySprite' + (i + 1)].node['roomUser'] = my;

                _this['mySprite' + (i + 1)].node.on(SystemEventType.TOUCH_END, _this.OnRoomUserClcik, _this);
              } else {
                _this['mySprite' + (i + 1)].spriteFrame = _this.normalBgSpriteFrame;

                _this['mySprite' + (i + 1)].node.off(SystemEventType.TOUCH_END, _this.OnRoomUserClcik, _this);
              }
            }

            for (var _i = 0; _i < _this.teamNum; _i++) {
              if (_i < enemyList.length) {
                var enemy = enemyList[_i];
                _this['enemySprite' + (_i + 1)].spriteFrame = yield (_crd && LoadResUtil === void 0 ? (_reportPossibleCrUseOfLoadResUtil({
                  error: Error()
                }), LoadResUtil) : LoadResUtil).load_local_sprite((_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
                  error: Error()
                }), DataManager) : DataManager).Instance.characters[enemy.cCharacterId].Avatar + '/spriteFrame');
                _this['enemyNicknameLabel' + (_i + 1)].string = enemy.nickName;
                _this['enemyNicknameLabel' + (_i + 1)].node.active = true;

                if (roomOwnerFlag) {
                  console.log('enemykickOutButton' + (_i + 1));
                  _this['enemykickOutButton' + (_i + 1)]['roomUser'] = enemy;
                  _this['enemykickOutButton' + (_i + 1)].active = true;
                } //头像点击


                _this['enemySprite' + (_i + 1)].node['roomUser'] = enemy;

                _this['enemySprite' + (_i + 1)].node.on(SystemEventType.TOUCH_END, _this.OnRoomUserClcik, _this);
              } else {
                _this['enemySprite' + (_i + 1)].spriteFrame = _this.normalBgSpriteFrame;

                _this['enemySprite' + (_i + 1)].node.off(SystemEventType.TOUCH_END, _this.OnRoomUserClcik, _this);
              }
            }
          })();
        }
        ...
        OnRoomUserClcik(param) {
          return _asyncToGenerator(function* () {
            console.log(param);
            var roomUser = param.currentTarget.roomUser;
        ...
        OnMyInviteClick() {
          return _asyncToGenerator(function* () {
            var uiInvite = yield (_crd && UIManager === void 0 ? (_reportPossibleCrUseOfUIManager({
              error: Error()
        ...
        OnBackClick() {
          ...
          return _asyncToGenerator(function* () {
            var roomOwnerFlag = _this2.room.userId == (_crd && User === void 0 ? (_reportPossibleCrUseOfUser({
              error: Error()
            }), User) : User).Instance.user.id;
            var tips = roomOwnerFlag ? '解散房间' : '退出房间';
        ...
        */
        ...
        OnkickOut(param, customEventData) {
          var _this3 = this;
