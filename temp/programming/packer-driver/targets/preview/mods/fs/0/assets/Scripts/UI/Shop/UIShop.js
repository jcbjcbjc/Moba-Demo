      ...
      _export("UIShop", UIShop = (_dec = ccclass('UIShop'), _dec2 = property(_crd && List === void 0 ? (_reportPossibleCrUseOfList({
        ...
        OnClickBuy() {
          ...
          return _asyncToGenerator(function* () {
            ...
            var itemDefine = _this.uiShopItem.itemDefine; // let confirmObj= await MessageBox.Show("确定要购买"+itemDefine.Name+"吗？", "购买", MessageBoxType.Confirm,"确定","取消"); 
            // let this_=this;
            ...
            _this.buyCountInputObj = yield (_crd && InputBox === void 0 ? (_reportPossibleCrUseOfInputBox({
              error: Error()
            }), InputBox) : InputBox).Show('请输入购买数量', "购买");
            _this.buyCountInputObj.input.string = '1'; //默认值
        ...
        */
        ...
        InitShop() {
          ...
          return _asyncToGenerator(function* () {
            _this2.shopItemList = [];
            var shopItems = (_crd && DataManager === void 0 ? (_reportPossibleCrUseOfDataManager({
              error: Error()
            }), DataManager) : DataManager).Instance.shopItems[_this2.index + 1];

            if (!shopItems || Object.keys(shopItems).length < 1) {
              _this2.uiShopDetailItem.node.active = false;
            } else {
              _this2.uiShopDetailItem.node.active = true;
              var i = 0;

              for (var key in shopItems) {
                var shopItem = shopItems[key];

                if (shopItem.Status > 0) {
                  _this2.shopItemList.push(shopItem);
            ...
            _this2.shopList.numItems = _this2.shopItemList.length;

            _this2.shopList.scrollTo(0);
          })();
        }
        ...
        */
        ...
        OnShopListRender(node, index) {
          var uiShopItem = node.getComponent(_crd && UIShopItem === void 0 ? (_reportPossibleCrUseOfUIShopItem({
            error: Error()
